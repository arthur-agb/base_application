...
const checkProjectMembership = (project, userId) => {
  ...
  // Correctly checks the userId on the MomentumProjectMember model
...
export const createSprint = async ({ projectKey, userId, companyId, title, goal, startDate, endDate }) => {
  ...
  const newSprint = await prisma.momentumSprint.create({
    data: {
      title,
      goal,
      status: 'PLANNED', // Added default status
  ...
  await MomentumHistoryModel.create({
    action: prisma.HistoryAction.CREATE,
    entityType: 'SPRINT',
 ...
 */
export const getAllSprintsByProject = async ({ projectKey, userId, companyId, status }) => {
  const project = await prisma.project.findFirst({
    where: { key: projectKey, companyId },
    include: { members: true },
  });
  ...
  if (!project || !checkProjectMembership(project, userId)) {
    return []; // Return empty array if not authorized, consistent with epic service
  }
 ...
 */
export const getSprintById = async ({ sprintId, userId, companyId }) => {
    const sprint = await prisma.momentumSprint.findUnique({
        where: { id: sprintId },
        include: {
            project: { where: { companyId }, include: { members: true } },
        },
    });
...
export const updateSprint = async ({ sprintId, userId, companyId, ...updateData }) => {
    ...
    const historyPromises = Object.keys(dataToUpdate).map(field => {
        return MomentumHistoryModel.create({
            action: prisma.HistoryAction.UPDATE,
            entityType: 'SPRINT',
            entityId: sprint.id,
            fieldChanged: field,
...
export const deleteSprint = async ({ sprintId, userId, userRole, companyId }) => {
    ...
    // Create History Record
    await MomentumHistoryModel.create({
        action: prisma.HistoryAction.DELETE,
        entityType: 'SPRINT',
        entityId: sprintId,
 ...
 */
export const getSprintIssues = async ({ sprintId, userId, companyId }) => {
    const sprint = await prisma.momentumSprint.findFirst({
        where: { id: sprintId, project: { companyId } },
        include: { project: { include: { members: true } } },
    });
 ...
 */
export const searchSprints = async ({ projectKey, query, userId, companyId }) => {
    const project = await prisma.project.findFirst({
        where: { key: projectKey, companyId },
        include: { members: true },
    });
